...
router.get('/', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const bills = await prisma_1.prisma.salesBill.findMany({
...
router.post('/', auth_middleware_1.authenticate, validation_middleware_1.validateCreateSale, async (req, res) => {
    try {
        const { customerId, customerName, customerPhone, doctorName, notes, paymentMethod, discount, items, isRoundOff, roundOffAmount } = req.body;
        ...
        const saleResult = await prisma_1.prisma.$transaction(async (tx) => {
            let subtotal = 0;
            let taxTotal = 0;
...
router.get('/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const bill = await prisma_1.prisma.salesBill.findUnique({
            where: { id },
            include: {
                customer: true,
                user: { select: { id: true, name: true } },
