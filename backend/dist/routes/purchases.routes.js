...
router.get('/', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const bills = await prisma_1.prisma.purchaseBill.findMany({
...
router.get('/next-number', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const latestBill = await prisma_1.prisma.purchaseBill.findFirst({
            orderBy: { createdAt: 'desc' },
        ...
        if (latestBill && latestBill.invoiceNumber) {
            ...
            if (match) {
                ...
                const count = await prisma_1.prisma.purchaseBill.count();
                nextSeq = count + 1001;
            }
        }
...
router.post('/', auth_middleware_1.authenticate, validation_middleware_1.validateCreatePurchase, async (req, res) => {
    try {
        ...
        const purchaseResult = await prisma_1.prisma.$transaction(async (tx) => {
            let subtotal = 0;
            let taxTotal = 0;
...
router.get('/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const bill = await prisma_1.prisma.purchaseBill.findUnique({
            where: { id },
            include: {
                party: true,
                items: {
