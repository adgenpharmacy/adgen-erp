...
function findJsonFile(baseDir: string, fileName: string): string | null {
  ...
  for (const item of items) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      const subPath = path.join(fullPath, fileName);
...
async function migrate() {
  ...
  const targetDir = fs.existsSync(rootDataDir) ? rootDataDir : backendDataDir;
  ...
  if (productsPath) {
    const docs = JSON.parse(fs.readFileSync(productsPath, 'utf-8')).docs || [];
  ...
  if (customersPath) {
    const docs = JSON.parse(fs.readFileSync(customersPath, 'utf-8')).docs || [];
  ...
  if (partiesPath) {
    const docs = JSON.parse(fs.readFileSync(partiesPath, 'utf-8')).docs || [];
  ...
  if (inventoryPath) {
    const docs = JSON.parse(fs.readFileSync(inventoryPath, 'utf-8')).docs || [];
  ...
  if (purchasePath) {
    const docs = JSON.parse(fs.readFileSync(purchasePath, 'utf-8')).docs || [];
  ...
  if (salesPath) {
    const docs = JSON.parse(fs.readFileSync(salesPath, 'utf-8')).docs || [];
    ...
    for (const bill of docs) {
      ...
      if (isNaN(invNum) || !invNum) invNum = 1000 + Math.floor(Math.random() * 9000);
