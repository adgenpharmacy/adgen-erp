  ...
  static Future<void> checkForUpdate(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        const currentBuild = AppVersion.buildNumber;
...
class _UpdateDialogState extends State<_UpdateDialog> {
  ...
  Future<void> _startDownload() async {
    ...
    try {
      // Use external storage directory for better package installer compatibility on Android
      ...
      // Fallback: Open URL in browser so user can download directly
      try {
        await launchUrl(Uri.parse(widget.data['downloadUrl']), mode: LaunchMode.externalApplication);
        if (mounted) Navigator.pop(context);
      } catch (_) {}
    }
  }
