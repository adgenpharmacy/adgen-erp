...
/// `mrp` and `rate` store the last-known defaults for convenience when
/// creating new purchase/sales entries. Actual values vary per batch.
class ProductModel extends Equatable {
  ...
  factory ProductModel.fromFirestoreMap(Map<String, dynamic> data, {String? id}) {
    final docId = id ?? data['id'] ?? data['_id'];
    ...
    return ProductModel(
      ...
      productType: ProductType.values.firstWhere(
        (t) => t.name.toLowerCase() == (data['productType'] ?? '').toString().toLowerCase(),
        orElse: () => ProductType.tablet,
      ),
      division: ProductDivision.values.firstWhere(
        (d) => d.name.toLowerCase() == (data['division'] ?? '').toString().toLowerCase(),
  ...
  factory ProductModel.fromFirestore(DocumentSnapshot doc) {
    return ProductModel.fromFirestoreMap(doc.data() as Map<String, dynamic>, id: doc.id);
  }
